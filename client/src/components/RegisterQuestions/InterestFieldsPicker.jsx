...
const InterestFieldsPicker = ({
  ...
  let badges = ["Web development", "DSA", "CP", "Android development", "Machine learning", "Cyber-security", "Artificial Intelligence", "Game development", "Cloud computing", "IoT", "User experience design"];
  ...
  const handleDelete = (e) => {
    const updatedInterests = interests.filter((title) => title !== e.target.innerText);
  ...
  return (<div className="flex justify-center items-center min-h-screen bg-pink-50">
    <div
    ...
    >
      ...
      >
        <h2 className="text-2xl font-medium underline mb-2">Fields of interest</h2>
        ...
        <div className="m-5 flex flex-wrap">
          ...
          {badges.map((title, index) => {
            ...
            return <div key={index} onClick={isPicked ? handleDelete : handlePick}
                        className={`badge border  shadow-sm border-stone-400 p-4 m-2 gap-2 cursor-pointer ${isPicked && "text-white bg-blue-500 border-0 shadow-xl"}`}>
              {isPicked && <RxCross2 />}
              {title}
            </div>;
          })}
